...
We've already see the most commonly used of these functions in our previous examples — the `eq` function. This function expects two or more arguments, and returns true if all the arguments have the same value, and false otherwise. Here's the full list of comparison functions:
